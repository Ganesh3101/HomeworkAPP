...
genai.configure(api_key = streamlit.secrets['GOOGLE_API_KEY'])
...
def extract_from_docs(file):
    ...
    text = '\n'.join([p.text for p in docs.paragraphs])
    return text
...
def get_gemini_response_img(img,prompt):
...
if uploaded_file:
    ...
    # Handle Word files
        ...
        extracted_text = extract_from_docs(uploaded_file)
    # Handle Image files
    ...
    with col2:
        ...
        try:
            if extracted_text:
                ...
                response = get_gemini_response_img(image, prompt)
